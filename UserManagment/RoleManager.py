...
from UserManagment import Roles
...
class RoleManager:
    ...
    async def assign_role(self, band_member: BandMember, role: Roles.RoleWrapper) -> None:
        self._check_valid(band_member, BandMember)
        self._check_valid(role, Roles.RoleWrapper)
        await band_member.member.add_roles(role.raw) # role just acts like a discord.Role, we pass in the real role with .raw

    async def remove_role(self, band_member: BandMember, role: Roles.RoleWrapper) -> None:
        self._check_valid(band_member, BandMember)
        self._check_valid(role, Roles.RoleWrapper)
        real_member = band_member.member
        if role.raw in real_member.roles:
            await real_member.remove_roles(role.raw)
        else:
            raise Exception(f"Could not find role {role} in {band_member.name}'s roles.\n({real_member.roles})")

    async def remove_category(self, band_member: BandMember, category: Roles.MarchingRole.__subclasses__()) -> None:
        roles = band_member.member.roles
        for role in category:
            if role.raw in roles:
                await band_member.member.remove_roles(role.raw)
